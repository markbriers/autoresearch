...
- **Wider MLPs improve val_bpb under step-bounded training.** 4x→6x→8x MLP expansion each improved val_bpb (-0.005, then -0.002 more). ReluSquared creates sparse activations; wider MLPs provide more capacity to separate the sparse signal. The v3 failure of 6x MLP was entirely due to throughput bias. (v4 Runs 2, 5)

- **Depth scaling with 8x MLP is extremely powerful.** 10→12 layers gave -0.032 val_bpb (the single biggest improvement in v4). 12→14 gave another -0.020. The model was severely depth-starved at 10 layers. Depth > width at this scale with 8x MLP. Diminishing returns: 10→12 = -0.032/layer, 12→13 = -0.017, 13→14 = -0.003. D_crit appears to be around 14-15 for W=640 with 8x MLP. (v4 Runs 7-10)

- **Depth 14 with 8x MLP exceeds 20-min safety limit.** Depth=12 is the max that fits in ~16 min. Depth=13-14 require 23-25 min. If wall-clock limit can be relaxed, depth=14 at val_bpb 0.895 is optimal. (v4 Runs 7-10)

- **10x MLP at depth=12 fits in time budget and improves further.** 8x→10x MLP gives -0.002 val_bpb at depth=12. The optimal time-budget configuration is now 12×640×10x MLP, 220M params, 18.9 min. (v4 Run 15)

- **Weight tying fails catastrophically with this optimizer.** The embedding_lr (0.6) is 150x the unembedding_lr (0.004). Tying weights forces a single LR, destroying the output projection. Weight tying requires matching input/output LRs. (v4 Run 14)

- **LayerDrop is incompatible with torch.compile.** Data-dependent layer skipping breaks static graph compilation, causing 2x slowdown. Without compile, depth=14 would need 49 min. torch.compile is essential for this architecture. (v4 Run 13)

- **z-loss is redundant with softcap.** Softcap=12 already constrains logits effectively. Adding z-loss (1e-4) gives no improvement. (v4 Run 11)

- **SwiGLU beats ReluSquared-GLU by -0.0036 at iso-params.** At 8x expansion with depth=12, SwiGLU (SiLU gate, dense) outperforms ReluSquared-GLU (sparse). SiLU's smooth gradients give the model access to the full hidden dim; ReluSquared's ~95% sparsity wastes most neurons. At iso-FLOP, SwiGLU 6x matches ReluSquared 8x. (v4 Runs 17-20)

- **SwiGLU 6x ≈ ReluSquared 8x at iso-depth.** At depth=14, SwiGLU with 6x expansion (313M params) matches ReluSquared with 8x expansion (291M params) at val_bpb ~0.895. The gating mechanism compensates for less raw capacity. (v4 Run 20 vs Run 8)
...
- **DiffAttn is fundamentally incompatible at sub-250M scale.** Both fixed lambda (Run 3: val_bpb 1.830) and learnable lambda reparameterization (Run 16: val_bpb 1.829) are catastrophic. The problem is NOT lambda — it's that splitting Q/K into two 64-dim halves reduces per-head attention capacity below a viability threshold at this scale. With only 5-6 heads, each head needs its full 128 dimensions. The DiffAttn paper's smallest model is 830M. At sub-250M, DiffAttn is a dead end. (v4 Runs 3, 16)

- **Anticorrelated noise injection conflicts with Muon's momentum.** Anti-PGD (sigma=0.001) on Muon params gave +0.004 regression vs 6x MLP baseline. Muon's Nesterov momentum creates carefully structured temporal correlations in updates; anticorrelated noise disrupts these. Anti-PGD was designed for vanilla GD and never tested with momentum optimizers. (v4 Run 4)

- **Mousse curvature preconditioning doesn't help at 85M scale.** Left-only Kronecker preconditioning with spectral tempering (alpha=0.125) gave +0.001 regression. At this model size, the curvature may be insufficiently anisotropic to benefit from preconditioning. NorMuon's existing per-neuron variance tracking may already capture the relevant structure. (v4 Run 1)

- **Muon REQUIRES long warmdown — WSD short-cooldown does not transfer from AdamW.** Reducing warmdown from 70% to 20% caused +0.007 regression. Muon's orthogonal updates (NS projection) create larger oscillations than AdamW's adaptive updates. The long cosine warmdown is actively smoothing these oscillations, not just "revealing" progress. The WSD paper's findings are AdamW-specific. 70% warmdown is load-bearing for Muon. (v4 Run 6)
...
- ~~**Does MLP expansion ratio matter?**~~ **RESOLVED: MLP expansion is highly beneficial under step-bounded training.** 4x→6x→8x each improved val_bpb. The v3 failure was entirely throughput bias. 8x gives -0.007 vs 4x baseline. Diminishing returns: 4x→6x = -0.005, 6x→8x = -0.002. (v4 Runs 2, 5)
