...
Your concern about compounding results making further innovation harder is well-founded. Each confirmed improvement changes the baseline, and stacking failures in v8 showed that improvements can interfere destructively. The design space narrows as the model accumulates modifications, and the risk of overfitting to the validation set grows with every experiment. Whether H4's derivative correction will compose with future architectural changes or create the same stacking problems as v8 is an open question.
...
| v9 | 1800 steps, frozen HPs | 5 | 5 | -0.004 (research) | Bayesian surprise; PD residual scaling (novel) |
...
**In v9, the system produced what appears to be a genuinely novel mechanism.** PD-controller residual scaling — computing the inter-layer velocity of the residual stream and applying a learned derivative correction — has no direct published precedent that two independent comprehensive searches could find. The closest prior art (Momentum ResNets, PIDformer) uses different mechanisms with different theoretical motivations. Both agents predicted this hypothesis would fail. The Bayesian surprise framework identified it as the most informative outcome precisely because both agents' models were wrong. The system found something new not by looking for novelty, but by testing what neither agent believed in.

**Whether this constitutes "research" is now a cleaner question.** In v5-v8 the answer depended on definition: novel reasoning paths to known mechanisms, maybe research, maybe not. In v9 the mechanism itself appears novel (pending more thorough literature review). A single confirmed novel result across 200+ experiments and nine protocol iterations is not a high hit rate. But it is a non-zero hit rate, and the protocol that produced it (Bayesian surprise integration, independent probability estimates, high-surprisal prioritisation) is well-characterised and reproducible.
...
**Single-line summary:** Across nine versions, the system produced one apparently novel mechanism (PD residual scaling from control theory, v9), several novel reasoning paths to known mechanisms (partial RoPE via OFDM, ShrinkReLU via wavelet shrinkage), and a large body of well-documented negative results. Most of the work was in the harness: the system prompts, the tool restrictions, the context isolation between subagents, the persistent memory format, the cross-domain enforcement, and the Bayesian surprise integration that ultimately surfaced the one result neither agent believed in.

---

The code, results files, research logs, and hypothesis records across all nine versions are at [github.com/markbriers/autoresearch](https://github.com/markbriers/autoresearch) on the `autoresearch/mar23` branch. Total compute cost: approximately £100. Karpathy's original is at [github.com/karpathy/autoresearch](https://github.com/karpathy/autoresearch).
